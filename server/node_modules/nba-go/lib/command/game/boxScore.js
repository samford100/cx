...
const checkOverStandard = (record, standard) => +record >= standard ? (0, _log.neonGreen)(record) : record;

const checkGameHigh = (players, record, recordVal) => {
  const recordArr = players.map(player => Number.parseInt(player[record], 10));
  return recordVal >= Math.max(...recordArr) ? (0, _log.neonGreen)(recordVal) : recordVal;
};
...
const createTeamBoxScore = team => {
  ...
  players.forEach(player => {
    ...
    boxScoreTable.push(alignCenter([(0, _log.bold)(`${first_name} ${last_name}`), (0, _log.bold)(position_short), checkGameHigh(players, 'minutes', minutes), `${field_goals_made}-${field_goals_attempted}`, `${three_pointers_made}-${three_pointers_attempted}`, `${free_throws_made}-${free_throws_attempted}`, checkGameHigh(players, 'plus_minus', plus_minus), checkGameHigh(players, 'rebounds_offensive', rebounds_offensive), checkGameHigh(players, 'rebounds_defensive', rebounds_defensive), checkOverStandard(totalRebounds, 10), checkGameHigh(players, 'assists', assists), checkGameHigh(players, 'steals', steals), checkGameHigh(players, 'blocks', blocks), checkGameHigh(players, 'turnovers', turnovers), checkGameHigh(players, 'fouls', fouls), checkGameHigh(players, 'points', points)]));
  });
